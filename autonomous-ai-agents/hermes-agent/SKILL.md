...
Hermes has a built-in web dashboard on port **9119** for managing config, API keys, sessions, and (optionally) an embedded chat tab.

```bash
hermes dashboard                    # Start (localhost:9119)
hermes dashboard --status           # Check running instances
hermes dashboard --stop             # Stop dashboard
hermes dashboard --host 0.0.0.0 --insecure --no-open   # External access
```

For external access setup (API Server + Dashboard with IP whitelist via iptables + cloud security groups), see [`references/api-server-setup.md`](references/api-server-setup.md).
For dashboard-specific auth (nginx auth_basic proxy with token login), see [`references/dashboard-nginx-proxy.md`](references/dashboard-nginx-proxy.md).
...
For providers not listed in the interactive picker, or when you need fine-grained control, see [`references/manual-provider-setup.md`](references/manual-provider-setup.md) — covers manual .env + config.yaml edits, model discovery API, and provider section setup.
...
### Achievements (gamified tracking)

The `hermes-achievements` plugin tracks 60+ achievements from real session
history — tool usage, model diversity, debugging patterns, lifestyle metrics,
and more. Data is persisted to the filesystem and available without the
dashboard. When the dashboard is behind auth, query progress data directly:

| File | Description |
|------|-------------|
| `~/.hermes/plugins/hermes-achievements/state.json` | Permanent unlock records (tier, timestamp) |
| `~/.hermes/plugins/hermes-achievements/scan_snapshot.json` | Current progress snapshot (per-achievement metrics, thresholds) |

For query patterns and schema, see
[`references/achievements-plugin-data.md`](references/achievements-plugin-data.md).

For a step-by-step procedure to audit model/provider diversity (Provider Polyglot,
Five-Model Flight, Multi-Model Mage thresholds, testing workflows, and unlock
paths for missing providers), see
[`references/model-diversity-audit.md`](references/model-diversity-audit.md).
...
### Stale `.pyc` → ImportError on inbound messages

**Symptom** — All platforms show `✓ connected` in `gateway.log`, but every inbound message
fails with `ImportError: cannot import name '<X>' from 'tools.<module>'`. Direct
`python -c "from tools.<module> import <X>"` from the venv **succeeds**. Error only
appears inside the gateway's per-message subprocess (which re-imports `run_agent`
→ tool modules on every message).

**Root cause** — Stale `.pyc` cache + partial `sys.modules` import. After `hermes update`,
a hot-reload, or a partial install that lands new `.py` without restarting, Python can
cache a half-imported module into `sys.modules`. The main gateway process imported a
working copy at startup; per-message subprocesses re-import via the stale `.pyc` and
land on the broken partial. CLI from a fresh shell uses a clean `sys.modules` so it
bypasses the cache.

**Fix (2 commands, <30s)**
```bash
# 1. Invalidate stale bytecode for the failing module (+ any module in its import chain)
rm ~/.hermes/hermes-agent/tools/__pycache__/<module>.cpython-311.pyc

# 2. Restart gateway — clears sys.modules in the long-lived process
hermes gateway restart
```

**Verify (don't skip — direct CLI import is NOT proof of fix)**
```bash
# Walk the actual import chain the gateway walks
cd ~/.hermes/hermes-agent && ./venv/bin/python -c "
from tools.<module> import <X>
from tools.terminal_tool import cleanup_vm
import run_agent
print('OK')
"
# Then send one message from the messaging platform and confirm no ImportError
tail -20 ~/.hermes/logs/gateway.log
```

**Pitfalls**
- Only deleting the `.pyc` is NOT enough — the long-lived process still has the bad
  module in `sys.modules`. Restart is required.
- Only restarting is NOT enough — Python will re-read the stale `.pyc` on next
  subprocess spawn and re-break. Both steps are required.
- `python -c "from tools.X import Y"` from a fresh shell is the **wrong verify** — it
  uses your shell's clean `sys.modules`, not the gateway's cached one. Always include
  `import run_agent` in the smoke test to mirror the gateway's import path.
- See [`references/gateway-import-error-recovery.md`](references/gateway-import-error-recovery.md)
  for full diagnostic recipe, distinguishing table, and transcript.

### Platform-specific issues
- **Discord bot silent**: Must enable **Message Content Intent** in Bot → Privileged Gateway Intents.
- **Discord bot ignores other bots**: Set `DISCORD_ALLOW_BOTS=mentions` in `~/.hermes/.env`.
  - For multi-bot meeting protocol (plannerbot + 채니봇 + aiprofit), see [`references/discord-multi-bot-meetings.md`](references/discord-multi-bot-meetings.md).
  - `"none"` (default) → ignores all other bot messages (multi-bot meetings break)
  - `"mentions"` → responds when another bot @mentions you ✅ (required for plannerbot meetings)
  - `"all"` → responds to every bot message (spammy)
  - **After changing .env, restart gateway**: `hermes gateway restart` or `systemctl --user restart hermes-gateway`
  - **Verify**: Run inside gateway: `python3 -c "import os; print(os.environ.get('DISCORD_ALLOW_BOTS','MISSING'))"`
  - **Note**: `.env` vars loaded by `load_hermes_dotenv()` at runtime are NOT in `/proc/PID/environ`. Use the python check above, not `grep` on /proc.
