...
version: 1.26.0
changelog:
  - "1.26.0 (2026-08-17): (a) P19 15회 연속 재발 — 첫 04:00 auto-sync 편승 committed+push 변형: 08-16 오염분 58건이 d36d3b3 (auto-sync 08-17 04:00)에 편승해 HEAD+origin/main 모두 오염 (전날 076bafb가 58줄 제거했는데 4시간 뒤 재삽입). P19는 주간 cleanup뿐 아니라 매일 2회 auto-sync 커밋에도 편승 가능 — git log 최근 커밋을 의심 대상에서 제외하지 말 것; (b) 복구는 parent 복원 경로 재사용 (git show 076bafb:index.md > index.md, commit 4afdc74, push d36d3b3..4afdc74); (c) untracked raw draft 없는 케이스 확인 (W34 미발생)"
...
#      커밋/푸시 불필요 (working-tree 오염만 복구).
#      git diff HEAD에 변경이 남아 있으면 → 커밋 필요.
git diff HEAD -- index.md
# 빈 결과: "P19 working-tree 전용 오염 — 커밋 불필요"
# 변경 있음: "P19 committed 오염 — git add + commit + push 필요"

# 추가: HEAD 상태 확인 (git diff만으로는 submodule pointer 변경 등 간과 가능)
