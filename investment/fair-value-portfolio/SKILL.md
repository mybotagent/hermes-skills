...
- **Moat 점수는 LLM이 종목 데이터로 1~10 추정**: 해자 요소(특허·독점, 네트워크효과, 전환비용, 브랜드파워, 규제장벽) 기준
  - 9~10: 독점적 지위 (MSFT, GOOGL)
  - 7~8: 강한 시장지위 (NVDA, AVGO — AI 경쟁 심화로 2026-07-17 하향)
  - 5~6: 차별화 보통
  - 3~4: 차별화 약함
  - 1~2: 진입장벽 없음
- **오직 gap + moat 두 가지 요소만 사용** — PER상태/성장성/현금흐름 등 다른 요소 반영 금지
- **SELL 판정 종목**: 비중 = 2% (계산에서 제외)
- **저승률 조기 경보 (2026-07-17 신규)**: 승률 < 30% AND 보유 5일 초과 → LLM 재평가. 3일 연속 -5% 이상 → 즉시 HOLD 검토. 10일 중 8일 하락(<20%) → SELL 권장.
...
**현금 비중 규칙 — 유연한 조정** (2026-06-08 수정, 2026-07-17 회고 반영):
- Before: 하드코딩 (Slowdown=25% 고정, Alpha-Flip ±5%p만 허용)
- After: Regime 기본값 + Alpha-Flip 보정 + 뉴스/매크로 심각성 반영
  - Regime 기본: Goldilocks 5% / Overheat **10%** / Slowdown **20%** / Stagflation 40%
  - Alpha-Flip: Bearish +5%p, Bullish -5%p
  - 추가 조정: 극단적 이벤트(전쟁, 반도체 쇼크, 유가 급등 등) 시 +10~20%p 추가 가능
  - MDD 선제 방어: Alpha-Flip Bearish + 연준/지정학 충격 동시 발생 → **즉시 현금 50%** (단계 금지)
  - 최대 현금 50%까지 가능
  - cash_reason에 구체적 근거 명시 (예: "Slowdown 25% + AlphaBearish +5%p + Broadcom쇼크 +10%p = 40%")

### 🔴 Portfolio Performance Feedback (2026-07-17) — 전략 개선

**실적**: Portfolio +0.79% vs S&P 500 +1.82% (28일, MDD -12.96%)

**문제 1 — 상승장 현금 22.3%는 기회비용**: S&P +1.82% 상승 동안 현금 22.3%가 0%.
- Overheat 기본 현금 15%→**10%** 로 하향
- Slowdown 25%→**20%**

**문제 2 — 저승률 종목 조기 퇴출 실패**: LG이노텍(18% 승률) -1.77%, HPE(38%) -1.20%
- 승률 < 30% AND 보유 5일 초과 → LLM 재평가 플래그
- 3일 연속 -5% 이상 → 즉시 HOLD 검토
- 10일 중 8일 하락 → SELL 권장

**문제 3 — 엔비디아 12.6% 최대 비중인데 기여도 -0.04%**
- AI Infrastructure moat 점수 하향: NVDA 9~10→**7~8** (AI CapEx 둔화·경쟁 심화)
- AVGO 9~10→**8** (Broadcom 쇼크 반영)

**문제 4 — MDD 선제 대응 실패 (-12.96%, 6/22~7/13)**
- Alpha-Flip Bearish + 연준/지정학 충격 동시 발생 → **즉시 현금 50%** (단계 금지)
- MDD 8% 도달 시 2일 내 현금 40% 이상
- Regime 전환 감지 → 1일 내 현금 조정 완료
...
- **2026-07-17 v6: Multi-Factor Quality Scoring + Bear/Base/Bull 시나리오** — tradingAgents 분석 기반. Phase 3 프롬프트에 5개 Quality Factor(Reliability/Moat/Stability/Growth/Risk) + 종합 점수 → 등급(durable/high_quality/constructive/fragile/low_conviction) + Bear/Base/Bull 시나리오 + Return/Risk ratio 포함
...
## 📈 Paper Portfolio Performance Tracking (2026-07-16 신규, 07-17 보강)

로그된 일일 포트폴리오 스냅샷(`logs/portfolio/*.json`)을 yfinance 가격 데이터로 백필하여 누적수익률·MDD·Sharpe를 계산. HTML 대시보드(Chart.js, 다크테마)로 시각화.

- **스크립트**: `scripts/paper_tracker.py`
- **대시보드**: `data/portfolio_dashboard.html`
- **참조**: `references/paper-portfolio-tracker.md`
- **실행**: `cd ~/trade-pipeline && python3 scripts/paper_tracker.py`
- **출력 CSV**: daily.csv(일일/누적), holdings.csv(종목별 상세), market.csv(시장분위기), sp500.csv(S&P 비교)

### 대시보드 주요 기능
- 누적수익률 + S&P 500 비교 오버레이 (노랑 점선)
- 일일별 종목 breakdown 어코디언 (비중/수익률/기여도/사유)
- 시장 분위기 표시 (Regime 배지 + 시장 요약)
- 30일 페이지네이션 (◀ ▶)
- 모바일 반응형 + 다크 테마

### 🔴 함정 (2026-07-17)
- **old code 잔여 (🔴🔴)**: try/catch 리팩토링 시 아래 old code 삭제 확인. grep으로 1회 참조 확인할 것
- **data 선언 순서**: SP500 fetch는 data 변수 선언 후 실행. `spCum = data.map(...)`가 `const data = ...`보다 앞서면 ReferenceError
- **파일 권한**: dashboard HTML을 `chmod 644`로 해둘 것 (600이면 HTTP 서버 차단 위험)
- **시장 맥락 reason**: 개별 종목 reason에서 `gap XX% × moat Y/10 — ` 접두사 제거. `—` 이후만 표시
...
- **`references/cnbc-xml-naver-polling-workflow.md`** — CNBC XML REST API + Naver Polling API 검증된 실행 명령어 + 6종목 코드 (2026-09-10)
...
### 42. 🔴 기존 인프라 우선 활용 — 새로 만들기 전에 이미 있는 것부터 확인 (2026-07-17 신규)
- 사용자 교정: "기존의 거시경제 그래프 분석하는 거 있잖아 그거 사용하면 안될까?"
- 새 데이터 수집 파이프라인을 만들기 전에 기존 크론/cron 출력/macro_context.json/macro_strategy_report.py의 출력물을 먼저 확인할 것
- pandas_datareader로 FRED 데이터를 수집하는 macro_strategy_report.py가 이미 존재함 — 새로 만들지 말고 이 스크립트의 방식을 재사용
- **확인 순서**: data/macro_context.json → langgraph/src/macro_strategy_report.py → data/fair_value_stdout.txt → logs/portfolio/*.json

### 43. 🔴 tradingAgents 기반 Multi-Factor Quality Scoring (2026-07-17)
- Phase 3 프롬프트에 5개 Quality Factor 도입 (각 0~100점):
  - Reliability(25%): 출처 품질, 데이터 신선도
  - Economic Moat(25%): 특허·독점, 네트워크효과, 전환비용
  - Structural Stability(15%): 부채비율, 시장지위
  - Growth Quality(20%): ROE 추세, 이익률
  - Risk-Adjusted(15%): 변동성, 경쟁 리스크
- 등급: durable_compounder(85+)/high_quality(70-84)/constructive(55-69)/fragile(40-54)/low_conviction(0-39)
...
### 56. 🔴 일일 리포트 표현 검증 — 연속 기록/레벨은 전일 리포트 실제 값과 cross-check (2026-08-06 신규)

일일 매크로/브리핑 리포트에서 "나흘 연속 상승", "3일째 최고치", "800선 지지/회복" 같은 **연속성·레코드·레벨 표현**을 쓸 때는 반드시 전일 리포트의 실제 값(기존 `macro_context.json`의 지수 close, `kr_stocks`의 전일 종가, CNBC `pdc`)과 대조할 것.

**실제 사례 (2026-08-06)**: KOSDAQ 8/5 종가 799.59(<800)인데 리포트 초안에 "나흘 연속 800선"이라고 작성 → 800선을 넘은 건 8/6(+2.08 → 801.67)이 처음이므로 "800선 회복"으로 정정.

**검증 순서**:
1. 표현이 streak/레코드/레벨 터치를 주장하면 → 전일 리포트(직전 macro_context.json, 백업 .bak 파일)에서 기준값 확인
2. 레벨 표현(예: 800선)은 `pdc`(전일 종가)와 직접 비교 — `pdc >= 레벨`이어야 "연속/지지" 성립
3. 연속 상승일수는 전일 리포트의 "N일째" 카운트 +1인지만 확인 (금일 상승일 때)
...
# 영어:   hl=en-US&gl=US&ceid=US:en   ← 한국어 뉴스에서 안 나오는 1차 원인 확인용
# 실전 쿼리: 'Federal Reserve rate cut', 'S&P 500 stock market', 'Nvidia AI', 'Hormuz strait oil'
```
...
- 1.0 미만 → 비중 축소 검토
- Phase 3 출력: scenarios 객체 + return_risk_ratio
