...
- **에이피알(APR) 278470 (화장품 ODM) vs 천보 278280 (2차전지 부품)** — 2026-07-14 watchlist.json 정정 사례. `278280`을 Naver Polling으로 조회 시 `nm='천보'`로 응답 → 사용자 의도(에이피알)와 불일치. **정정 코드 278470**으로 재조회 시 `nm='에이피알'` 확인. 두 회사는 사업군·시총·변동성이 모두 다르므로 **데이터 무결성 위협**. 1회 응답으로 종목명 검증 필수.

**자동 정정 워크플로우** (watchlist.json에 잘못된 코드가 등록된 경우):
1. Naver Polling 1회 응답의 `nm` 필드와 사용자 의도 종목명 매칭
2. 불일치 시 → `grep -rn '278280\|278470' data/watchlist.json` + 관련 메모리/wiki cross-check
3. 사용자 확인 후 watchlist.json 단일소스 정정 (Pitfall 26 단일소스 원칙 준수)
4. 정정 후 동일 코드로 재조회하여 `nm` 일치 확인
5. **에이피알-천보처럼 코스피/코스닥 섞인 혼동**도 가능 (`.KS` ↔ `.KQ` suffix 검증 필수)

**Change_pct 부호 검증** (수집 후): Naver Polling의 `cr` 필드는 부호 포함된 문자열이지만, 직접 계산 시 `(nv - pcv) / pcv * 100` 결과와 부호가 일치하는지 확인. 예: 2026-07-14 삼성전기 `pcv=1,289,000`, `nv=1,260,000` → `cv=-29,000` (음수). cron에서 빠르게 표 작성 시 부호 누락하는 실수 방지.
