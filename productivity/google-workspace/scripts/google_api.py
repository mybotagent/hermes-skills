...
def get_credentials():
    """Load credentials: service account first, then fall back to OAuth."""
...
def calendar_list(args):
    cal_id = _resolve_calendar_id(args)
...
def _resolve_calendar_id(args) -> str:
    """When using a service account, override 'primary' to the shared user calendar."""
    cal_id = args.calendar
    if cal_id == "primary" and (HERMES_HOME / "google_service_account.json").exists():
        # Check for configured shared calendar email
        shared_cal = HERMES_HOME / "google_calendar_user.txt"
        if shared_cal.exists():
            return shared_cal.read_text().strip()
        return "tkd1496@gmail.com"
    return cal_id
...
def calendar_delete(args):
    cal_id = _resolve_calendar_id(args)
    if _gws_binary():
        _run_gws(["calendar", "events", "delete"], params={"calendarId": cal_id, "eventId": args.event_id})
